...
typedef struct __mavlink_cellular_status_t {
 ...
 uint16_t channel_number; /*<  The channel number (CN). Absolute radio-frequency (ARFCN) / E-UTRA (EARFCN) / UTRA (UARFCN) / New radio (NR_CH).*/
 float rx_level; /*< [dBm] On 3G is Received Signal Code Power (RSCP). On LTE Reference Signal Received Power (RSRP). On 5G  is New Radio Reference Signal Received Power (NR_RSRQ).*/
 ...
 * @param rx_level [dBm] On 3G is Received Signal Code Power (RSCP). On LTE Reference Signal Received Power (RSRP). On 5G  is New Radio Reference Signal Received Power (NR_RSRQ).
 ...
 *
 * @return [dBm] On 3G is Received Signal Code Power (RSCP). On LTE Reference Signal Received Power (RSRP). On 5G  is New Radio Reference Signal Received Power (NR_RSRQ).
 */
 ...
 *
 * @return  The bit error rate (BER) is determined by comparing the erroneous bits received with the total number of bits received. It is a ratio.
 */
 ...
 *
 * @return  Cellular instance number. Indexed from 1. Matches index in corresponding CELLULAR_MODEM_INFORMATION message.
 */
