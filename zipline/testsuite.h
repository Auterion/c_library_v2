...
static void mavlink_test_trisonica_wind(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_TRISONICA_WIND >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_trisonica_wind_t packet_in = {
        17.0,45.0,73.0,101.0,129.0,157.0,185.0,213.0
    };
    mavlink_trisonica_wind_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_TRISONICA_WIND_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_TRISONICA_WIND_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_trisonica_wind_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_trisonica_wind_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_trisonica_wind_pack(system_id, component_id, &msg , packet1.wind_n , packet1.wind_e , packet1.wind_d , packet1.temperature , packet1.humidity , packet1.pressure , packet1.wind_speed , packet1.wind_heading );
    mavlink_msg_trisonica_wind_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_trisonica_wind_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.wind_n , packet1.wind_e , packet1.wind_d , packet1.temperature , packet1.humidity , packet1.pressure , packet1.wind_speed , packet1.wind_heading );
    mavlink_msg_trisonica_wind_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_trisonica_wind_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_trisonica_wind_send(MAVLINK_COMM_1 , packet1.wind_n , packet1.wind_e , packet1.wind_d , packet1.temperature , packet1.humidity , packet1.pressure , packet1.wind_speed , packet1.wind_heading );
    mavlink_msg_trisonica_wind_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_zipline(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_trisonica_wind(system_id, component_id, last_msg);
}
